/**
 * Taken from [brawltimeninja](https://github.com/schneefux/brawltimeninja/blob/master/scripts/scraper.js)
 * @copyright brawltime.ninja, schneefux, and contributors.
 * @license UNLICENSED
 */
// eslint-disable
...
async function main() {
	...
	const brawltimeNinjaStarpowerJSONFull = await fetch(
		"https://cube.brawltime.ninja/cubejs-api/v1/load?" + starpowerQuery.toString()
	...
	const brawltimeNinjaGadgetJSONFull = await fetch(
		"https://cube.brawltime.ninja/cubejs-api/v1/load?" + gadgetQuery.toString()
	).then((response) => response.json());
	...
	async function downloadFileToLocation(link, path) {
		...
		if (actualSize > 0) {
			try {
				...
				console.error("Cannot download " + link, err);
			}
		}
	...
	async function getBrawlerData(brawlerName) {
		try {
			...
			const brawlerHtml = await fetch(brawlerUrl)
			...
			if (brawlerUrlRU) {
				brawlerHtmlRU = await fetch(brawlerUrlRU)
					.then((response) => response.text())
					.catch(() => {});
			...
			// voice lines
			if (voiceLineSectionID && opts.downloadVoiceLines) {
			...
			const skinSectionDepth =
				skinSectionID in wtfBrawler.sections() ? wtfBrawler.sections()[skinSectionID].json()["depth"] : 0;
			let skinSections = [];
			...
			const skinCategories = skinSections.map((skinSection) => {
				...
				const skins = skinsElementKeys.map((skinElementKey) => {
					if (skinElementKey.includes("name")) {
						...
						if (opts.downloadSkins) downloadQueue.push({ link, path });
			...
			// pins (russian)
			if (brawlerUrlRU && opts.downloadPins) {
				const addedPins = [];
